...
7. Is there any endpoint/CLI to fetch request **status** or more detailed **diagnostics/logs** beyond the final error string?
   Today we only see `unexecutable` / `duplicate funding UTXO...` in `.build/*.raw` even though app validation passed, and it would help to know *which stage* failed (policy check vs input selection vs tx assembly, etc.).
